...
extern "C" void imgui_wasm_imgui_backend_render_draw_data(imgui_wasm_backend_t* backend,
    ...
    for (int i = 0; i < draw_data->CmdLists.Size; i++) {
        ...
        std::vector<imgui_wasm_draw_cmd_t> imgui_wasm_cmds;
        imgui_wasm_cmds.reserve(cmd_list->CmdBuffer.Size);
        for (int j = 0; j < cmd_list->CmdBuffer.Size; j++) {
            const ImDrawCmd& pcmd = cmd_list->CmdBuffer[j];
            imgui_wasm_draw_cmd_t cmd;
            cmd.clip_rect[0] = pcmd.ClipRect.x;
            cmd.clip_rect[1] = pcmd.ClipRect.y;
            cmd.clip_rect[2] = pcmd.ClipRect.z;
            cmd.clip_rect[3] = pcmd.ClipRect.w;
            cmd.texture_id = (uint64_t)(uintptr_t)pcmd.GetTexID();
            cmd.idx_offset = pcmd.IdxOffset;
            cmd.vtx_offset = pcmd.VtxOffset;
            cmd.elem_count = pcmd.ElemCount;
            imgui_wasm_cmds.push_back(cmd);
        }

        GImGuiWasmCore->add_draw_list(
            (const uint8_t*)cmd_list->VtxBuffer.Data, (uint32_t)cmd_list->VtxBuffer.Size,
            (const uint8_t*)cmd_list->IdxBuffer.Data, (uint32_t)cmd_list->IdxBuffer.Size,
            (int)sizeof(ImDrawIdx), imgui_wasm_cmds.data(), (uint32_t)imgui_wasm_cmds.size());
    }
