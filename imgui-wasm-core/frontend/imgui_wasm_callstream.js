...
//
// When the server uses call-stream transport (messages 0x07/0x08/0x09), this
// module drives a WASM-compiled Dear ImGui "twin" that replays the captured
// ImGui calls to regenerate draw data locally. The produced draw data has the
// same flat vtx/idx/cmd byte layout as the draw-data protocol, so the existing
// renderFromParsed (imgui_wasm.js:203) renders it unchanged.
//
...
//   - on each 0x07/0x08: run imgui_wasm_replay_frame, parse output, render
...
// Reconstruct a synthetic draw-data frame (0x01-style) from the WASM twin's
// flat draw output so the existing parseDrawLists + renderFromParsed render it
// unchanged. The twin output is: per-list [numVtx u32][numIdx u32][numCmd u32]
// [vtx][idx u32*][cmd...], with NO type byte and NO header. We prepend:
//   0x01, dpx,dpy,dsw,dsh,fbsx,fbsy (6 f32), numLists (u32)
...
// Handle a 0x07 call-stream frame. `canvas`, `parseDrawLists`, `renderFromParsed`,
// and `lastFrame`/`lastFrameLists` are provided by imgui_wasm.js.
...
function enqueueReplay(snapshot, canvas, hooks) {
    // Serialize WASM access. Network frames and local input can arrive while
    // module initialization/replay is awaiting, and the twin is stateful.
    replayChain = replayChain
        .then(() => replaySnapshot(snapshot, canvas, hooks))
        .catch((e) => console.error('[imgui_wasm] call-stream replay error:', e));
    return replayChain;
}
...
function handleCallstreamFrame(data, canvas, hooks) {
    ...
    // The native backend prefixes the authoritative persisted INI as
    // LoadIniSettingsFromMemory (opcode 81, payload_len u32, id u32, size u32).
    // Applying the same INI inside every ImGui frame resets transient
    // widget/render state. Replay it only when the interned settings blob
    // changes, then strip the length-framed call from identical batches and
    // local input replays.
    if (callBytes.length >= 6) {
        const callView = new DataView(callBytes.buffer, callBytes.byteOffset, callBytes.byteLength);
        const payloadLen = callView.getUint32(2, true);
        const recordLen = 6 + payloadLen;
        if (callView.getUint16(0, true) === 81 && payloadLen === 8 && callBytes.length >= recordLen) {
            const iniId = callView.getUint32(6, true);
    ...
    if (iniCallBytes) {
        ...
        enqueueReplay(iniSnapshot, canvas, hooks);
    }
    return enqueueReplay(snapshot, canvas, hooks);
}

// Handle a 0x08 LZ4-compressed call-stream frame: decompress, then treat as 0x07.
async function handleCallstreamCompressed(data, isDelta, canvas, hooks, lz4Decompress) {
    const dv = new DataView(data.buffer, data.byteOffset, data.byteLength);
    const uncompressedLen = dv.getUint32(1, true);
    const compressed = data.subarray(5);
    const decompressed = lz4Decompress(compressed, uncompressedLen);
    // The decompressed body is a 0x07 payload WITHOUT the leading type byte
    // (mirrors the 0x03/0x05 layout). Reattach the type byte.
    const rebuilt = new Uint8Array(decompressed.length + 1);
    rebuilt[0] = 0x07;
    rebuilt.set(decompressed, 1);
    await handleCallstreamFrame(rebuilt, canvas, hooks);
}
...
window.imgui_wasmCallstream = {
    ...
    handleCompressed: handleCallstreamCompressed,
