...
function lz4Decompress(src, uncompressedLen) {
    const dst = new Uint8Array(uncompressedLen);
    let si = 0, di = 0;
    const srcLen = src.length;

    while (si < srcLen && di < uncompressedLen) {
        let token = src[si++];
        ...
        for (let i = 0; i < litLen && di < uncompressedLen; i++) {
            dst[di++] = src[si++];
        }

        if (si >= srcLen) break;

        let offset = src[si] | (src[si + 1] << 8);
        si += 2;
        ...
        for (let i = 0; i < matchLen && di < uncompressedLen; i++) {
...
function renderFrame(data) {
    try {
        const parsed = parseDrawLists(data);
        lastFrame = parsed;
        lastFrameLists = parsed.lists;
        lastDpx = parsed.dpx;
        lastDpy = parsed.dpy;
        lastDsw = parsed.dsw;
        lastDsh = parsed.dsh;
        ...
        console.error("[imgui_wasm] renderFrame error:", e);
    }
}

// --- Call-stream transport (Phase 3) --------------------------------------
// The call-stream helper (imgui_wasm_callstream.js) drives a WASM ImGui twin that
// replays captured calls. Loaded lazily on the first 0x07/0x08/0x09 message so
// draw-data-only servers pay nothing.
...
function parseDeltaFrame(data) {
    ...
    const [numLists] = readU32(dv, off); off += 4;
    const [numChanged] = readU32(dv, off); off += 4;

    if (!lastFrameLists || lastFrameLists.length !== numLists) {
        return null;
    }

    const newLists = [];
    for (let i = 0; i < numLists; i++) {
        newLists.push(lastFrameLists[i]);
    }

    for (let c = 0; c < numChanged; c++) {
        const [listIdx] = readU32(dv, off); off += 4;
        const [numVtx] = readU32(dv, off); off += 4;
        const [numIdx] = readU32(dv, off); off += 4;
        const [numCmd] = readU32(dv, off); off += 4;
        ...
        for (let j = 0; j < numCmd; j++) {
            ...
            const cw = dv.getFloat32(off, true); off += 4;
            const [texId] = readU64(dv, off); off += 8;
            const [idxOff] = readU32(dv, off); off += 4;
            const [vtxOff] = readU32(dv, off); off += 4;
            const [elemCount] = readU32(dv, off); off += 4;
        ...
        if (listIdx < numLists) {
            newLists[listIdx] = { vtxRaw, idxRaw, cmds };
        }
    }

    lastFrameLists = newLists;
    lastDpx = dpx;
    lastDpy = dpy;
    lastDsw = dsw;
    lastDsh = dsh;

    return { dpx, dpy, dsw, dsh, fbsx, fbsy, numLists, lists: newLists };
}

function handleDeltaFrame(data) {
    try {
        const parsed = parseDeltaFrame(data);
        if (parsed) {
            lastFrame = parsed;
            renderFromParsed(parsed);
        } else {
            console.warn("[imgui_wasm] delta: could not apply, no last frame or list count mismatch");
        }
    } catch (e) {
        console.error("[imgui_wasm] handleDeltaFrame error:", e);
    }
}

function handleCompressedFrame(data, isDelta) {
    try {
        const dv = new DataView(data.buffer, data.byteOffset, data.byteLength);
        const uncompressedLen = dv.getUint32(1, true);
        const compressedPayload = new Uint8Array(data.buffer, data.byteOffset + 5, data.byteLength - 5);
        const decompressed = lz4Decompress(compressedPayload, uncompressedLen);

        if (isDelta) {
            handleDeltaFrame(decompressed);
        } else {
            const rebuilt = new Uint8Array(uncompressedLen);
            rebuilt.set(decompressed);
            renderFrame(rebuilt);
        }
    } catch (e) {
        console.error("[imgui_wasm] handleCompressedFrame error:", e);
    }
}
    ...
    ws.onmessage = (evt) => {
        ...
        switch (msgType) {
            case 0x06: {
                ...
                // Synchronize client-owned state only after messages can carry
                // the assigned id. This also refreshes it after reconnecting.
                resize();
            ...
            case 0x03:
                handleCompressedFrame(data, false);
                break;
            case 0x04:
                handleDeltaFrame(data);
                break;
            case 0x05:
                handleCompressedFrame(data, true);
                break;
            case 0x07: {
            ...
            case 0x08: {
                loadCallstreamHelper().then((cs) => {
                    cs.handleCompressed(data, false, canvas, callstreamHooks(), lz4Decompress);
                });
                break;
            }
            case 0x09: {
