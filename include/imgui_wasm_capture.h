...
// imgui_ws/tools/generate_bindings.py) call these functions to record
// each ImGui call's opcode + args into a thread-local buffer. The core
// (src/capture.cpp) implements them.
//
// Outside an active session (before init or after shutdown), every function
// here checks an atomic enabled flag and returns immediately, so the
// wrappers are zero-cost.
#pragma once
...
// --- Frame lifecycle (called by the core, not host code) ------------------
